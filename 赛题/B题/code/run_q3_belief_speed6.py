#!/usr/bin/env python3
"""Local Monte Carlo of the speed-6 belief policy.

Same seed family as the fullopt 10k (2027). Never talks to the official
simulator. Default n=10000; pass --n 200 for a short smoke.
"""
...
N = 10_000
FIDELITY = "high"
OUT = Path(__file__).resolve().parents[1] / "_figs" / "q3_belief_speed6_10k"
...
def run_one(index: int, master_seed: int, fidelity: str, policy: str) -> dict:
...
def _worker(payload: tuple[int, int, str, str]) -> dict:
...
def main() -> None:
    ...
    parser.add_argument(
        "--policy",
        choices=("belief", "belief_ctrl", "belief_struct", "belief_punch", "belief_delay"),
        default="belief",
    )
    ...
    payloads = [
        (index, args.seed, args.fidelity, args.policy) for index in range(args.n)
    ]
    rows: list[dict] = []
    started = time.time()
    print(
        f"{args.policy}  n={args.n}  fidelity={args.fidelity}  "
    ...
    snaps = [row["stats"].get("cover_snaps") or 0 for row in rows]
    cover_moves = [row["stats"].get("n_cover_moves") or 0 for row in rows]
    ...
    summary = {
        ...
        "cover_snap_mean": statistics.fmean(snaps),
        "cover_snap7_frac": sum(1 for c in snaps if c >= 7) / len(rows),
        "cover_move_mean": statistics.fmean(cover_moves),
    }
            ...
            writer.writerow(
                {
                    ...
                    "cover_snaps": stats.get("cover_snaps", ""),
                    "n_cover_moves": stats.get("n_cover_moves", ""),
    ...
    print(
        ...
        f"s/src={summary['s_per_source_mean']}  "
        f"walk={summary['walk_mean']:.0f}m  cover7={summary['cover7_frac']*100:.1f}%  "
        f"snap7={summary.get('cover_snap7_frac', 0)*100:.1f}%  "
        f"cm={summary.get('cover_move_mean')}  "
