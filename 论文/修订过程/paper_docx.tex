...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig11_flowchart.png}
...
\begin{itemize}
\item 问题一：问题一代码final\_修正版.py（半平面交 + 旋转卡壳 + Welzl + 覆盖判定 + 仿真检验）、问题一\_表格与图像.py、figs/（示向度、交会定位、角形区域图）。
\item 问题二：code/problem2\_model.py、problem2\_paper.py、problem1\_localize.py（几何引擎）、test\_problem2.py（14 项回归测试）、final\_refine.py、results/problem2\_summary.json、figs/（三张候选区域图）。
\item 问题三：02-上场代码/（problem3\_belief.py、problem3\_route\_solver.py、run\_official\_e15.py 等）、新数据/（万局 summary.json、trials.csv）、04-正式测试-表1/（三份正式日志 formal-p3-*.jlog）。
\item 问题四：code/problem4\_belief\_n19.py（N19 主策略）、run\_official\_n19.py、01\_论文汇总表.csv、02\_按源数分组表.csv、03\_一万局逐局原始数据.csv、04\_未满清20局明细.csv、05\_原始统计汇总.json、正式日志 formal-p4-*.jlog。
\item AI 工具使用详情：AI 工具使用详情.pdf。
\end{itemize}
