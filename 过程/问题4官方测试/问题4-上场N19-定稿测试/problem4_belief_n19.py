...
Field copy: official HTTP injects robot=; local World still works.
v14 stays the rollback. This file never uses remaining() to stop.
"""
...
from problem4_batch_station import (
    bearing_point,
    clip_arena,
    item_center_bonus,
    possible_samples,
...
OUTER_N = 12
OUTER_ROT_DEG = 15.0
COVER_WAY_N = 0          # extra listen stops on a hop *to* an N19 cover (not N19 itself)
WAY_MIN_HOP = 400.0      # skip extra listens when the remaining hop is already short


def along_cover_segment(
    here: tuple[float, float],
    dest: tuple[float, float],
    n_way: int,
    min_hop: float = WAY_MIN_HOP,
) -> list[tuple[float, float]]:
    """Evenly spaced extra listen points on the way to a cover stop.

    Endpoints are excluded: dest is the N19 census point itself and must
    stay untouched. Short hops are skipped so we do not cluster listens.
    """
    if n_way <= 0:
        return []
    dist = math.dist(here, dest)
    if dist < min_hop:
        return []
    hx, hy = here
    dx = dest[0] - hx
    dy = dest[1] - hy
    return [
        (hx + dx * index / (n_way + 1), hy + dy * index / (n_way + 1))
        for index in range(1, n_way + 1)
    ]
...
def run_belief_q4(
    ...
    unheard_spacing: float = UNHEARD_SPACING,
    cover_way_n: int = COVER_WAY_N,
    gate_inner_unheard: bool = False,
    mute_transit_unheard: bool = True,
    sector_sweep_delay: bool = True,
    robot=None,
) -> dict:
    ...
    remaining0 = [
        (source.x, source.y)
        for source in getattr(world, "sources", [])
        if not getattr(source, "cleared", False)
    ]
    _, oracle_walk = two_opt_open((0.0, 0.0), remaining0) if remaining0 else (remaining0, 0.0)
    ...
    def should_measure(channel: int, way_listen: bool = False) -> bool:
        ...
        if not _heard(robot, channel):
            ...
            if cache is not None:
                if cover_listen == "kill":
                    useful = orient.silence_kills_cached(channel, cache)
                else:
                    useful = orient.hearable_cached(channel, cache)
                if not useful:
                    return False
                if not gate_inner_unheard:
                    return True
                # Outer ring stays exempt: a rim source facing out is often
                # only hearable there. Inner-ring re-listens of an already
                # silent channel are gated by unheard_spacing.
                if math.hypot(robot.x, robot.y) > INNER_RHO + 50.0:
                    return True
                return all(
                    math.dist(here, prev) >= unheard_spacing for prev in unused_listens
                )
            if mute_transit_unheard:
                return False
            if way_listen:
                return orient.hearable(channel, robot.x, robot.y)
    ...
    def work_here(expect: dict | None = None, way_listen: bool = False) -> None:
        nonlocal states, cover_at_first_clear
        snap_cover()
        here = (robot.x, robot.y)
        at_census = cache_here() is not None
        for channel in CHANNELS:
            if not should_measure(channel, way_listen=way_listen):
                continue
    ...
    def is_backward_far(target: tuple[float, float], pending_covers: list[tuple[float, float]]) -> bool:
        if not sector_sweep_delay:
            return False
        if not pending_covers:
            return False
        rx, ry = robot.x, robot.y
        dist = math.dist((rx, ry), target)
        if dist < 1300.0:  # Allow nearby clearing, prevent massive full-perimeter looping
            return False
        r_robot = math.hypot(rx, ry)
        if r_robot < 800.0:  # Always clear inner-ring targets immediately
            return False
        ang_robot = math.degrees(math.atan2(ry, rx)) % 360.0
        ang_target = math.degrees(math.atan2(target[1], target[0])) % 360.0
        delta_ccw = (ang_target - ang_robot) % 360.0
        if 90.0 <= delta_ccw <= 330.0:
            return True
        return False
    ...
    def tasks_here() -> list[dict]:
        tasks: list[dict] = []
        here = (robot.x, robot.y)
        pending_covs = [stop for _, stop in needed_covers()]
        for state in _clearable(states):
            ...
            if disk is None:
                continue
            if is_backward_far(disk[0], pending_covs):
                continue
        ...
        for state in states.values():
            ...
            if key in probed_keys:
                continue
            if is_backward_far(point, pending_covs):
                continue
            tasks.append(
                {
                    "kind": "probe",
        ...
        if unresolved:
            ...
            if not is_backward_far(station, pending_covs):
                tasks.append(
                    {
                        "kind": "station",
                        "channels": channels,
    ...
    while steps < max_steps:
        ...
        if task["kind"] == "cover" and cover_way_n > 0:
            for waypoint in along_cover_segment((robot.x, robot.y), dest, cover_way_n):
                robot.move_to(*waypoint)
                robot.n_replan += 1
                counters["way_stops"] += 1
                work_here(way_listen=True)
                if mission_complete():
                    stop_reason = "certificate"
                    break
            if stop_reason == "certificate":
                break
        robot.move_to(*dest)
        robot.n_replan += 1
    ...
    def _is_dir(source) -> bool:
        return getattr(source, "phi", None) is not None or bool(
            getattr(source, "directional", False)
        )

    n_dir = sum(1 for source in getattr(world, "sources", []) if _is_dir(source))
    dir_cleared = sum(
        1
        for source in getattr(world, "sources", [])
        if _is_dir(source) and source.cleared
    )
    return {
        "policy": "belief_q4",
        ...
        "way_stops": counters["way_stops"],
        "opp_unheard": counters["opp_unheard"],
        "cover_way_n": cover_way_n,
        "unheard_spacing": unheard_spacing,
        "gate_inner_unheard": gate_inner_unheard,
        "heard": n_heard(),
        "policy_framework": "n19",
    }


run_belief = run_belief_q4
